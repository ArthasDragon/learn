...
- [49 | 数据完整性（上）：硬件坏了怎么办？](#chapter49)
- [50 | 数据完整性（下）：如何还原犯罪现场？](#chapter50)
- [51 | 分布式计算：如果所有人的大脑都联网会怎样？](#chapter51)
...
<h1 id="chapter37">37 | 高速缓存（上）：“4毫秒”究竟值多少钱？</h1>
...
![chapter37-1](./imgs/chapter37-1.png)
...
![chapter37-2](./imgs/chapter37-2.jpg)
...
![chapter37-3](./imgs/chapter37-3.png)
...
![chapter37-4](./imgs/chapter37-4.png)
...
![chapter37-5](./imgs/chapter37-5.png)
...
![chapter37-6](./imgs/chapter37-6.png)
...
<h1 id="chapter38">38 | 高速缓存（下）：你确定你的数据更新了么？</h1>
...
![chapter38-1](./imgs/chapter38-1.jpeg)
...
![chapter38-2](./imgs/chapter38-2.jpeg)
...
![chapter38-3](./imgs/chapter38-3.jpeg)
...
<h1 id="chapter39">39 | MESI协议：如何让多核CPU的高速缓存保持一致？</h1>
...
我们说，CPU Cache 解决的是内存访问速度和 CPU 的速度差距太大的问题。而多核 CPU 提供的是，在主频难以提升的时候，通过增加 CPU 核心来提升 CPU 的吞吐率的办法。我们把多核和 CPU Cache 两者一结合，就给我们带来了一个新的挑战。因为 CPU 的每个核各有各的缓存，互相之间的操作又是各自独立的，就会带**来缓存一致性**（Cache Coherence）的问题。

![chapter39-1](./imgs/chapter39-1.jpeg)

## 缓存一致性问题

那什么是缓存一致性呢？我们拿一个有两个核心的 CPU，来看一下。你可以看这里这张图，我们结合图来说。

![chapter39-2](./imgs/chapter39-2.jpeg)
...
![chapter39-3](./imgs/chapter39-3.jpeg)
...
![chapter39-4](./imgs/chapter39-4.jpeg)
...
![chapter39-5](./imgs/chapter39-5.jpeg)
...
<h1 id="chapter40">40 | 理解内存（上）：虚拟内存和内存保护是什么？</h1>
...
![chapter40-1](./imgs/chapter40-1.png)
...
![chapter40-2](./imgs/chapter40-2.jpeg)
...
![chapter40-3](./imgs/chapter40-3.jpeg)
...
![chapter40-4](./imgs/chapter40-4.jpg)
...
![chapter40-5](./imgs/chapter40-5.jpeg)
...
![chapter40-6](./imgs/chapter40-6.jpeg)
...
<h1 id="chapter41">41 | 理解内存（下）：解析TLB和内存保护</h1>
...
![chapter41-1](./imgs/chapter41-1.jpeg)
...
![chapter41-2](./imgs/chapter41-2.jpeg)
...
![chapter41-3](./imgs/chapter41-3.jpeg)
...
于是，大家会在数据库里存储密码的哈希值，比如用现在常用的 SHA256，生成一一个验证的密码哈希值。但是这个往往还是不够的。因为同样的密码，对应的哈希值都是相同的，大部分用户的密码又常常比较简单。于是，拖库成功的黑客可以通过**彩虹表**的方式，来推测出用户的密码。
...
<h1 id="chapter42">42 | 总线：计算机内部的高速公路</h1>
...
![chapter42-1](./imgs/chapter42-1.jpeg)
...
![chapter42-2](./imgs/chapter42-2.jpeg)
...
![chapter42-3](./imgs/chapter42-3.jpeg)
...
![chapter42-4](./imgs/chapter42-4.jpeg)
...
我们的总线是很多个设备公用的，那多个设备都想要用总线，我们就需要有一个机制，去决定这种情况下，到底把总线给哪一个设备用。这个机制，就叫作**总线裁决**（Bus Arbitraction）。总线裁决的机制有很多种不同的实现，如果你对这个实现的细节感兴趣，可以去看一看 Wiki 里面关于裁决器的对应条目，这里我们就不多说了。
...
<h1 id="chapter43">43 | 输入输出设备：我们并不是只能用灯泡显示“0”和“1”</h1>

们在前面的章节搭建了最简单的电路，在这里面，计算机的输入设备就是一个一个开关，输出设备呢，是一个一个灯泡。的确，早期发展的时候，计算机的核心是做“计算”。我们从“计算机”这个名字上也能看出这一点。不管是中文名字“计算机”，还是英文名字“Computer”，核心都是在”计算“这两个字上。不过，到了今天，这些“计算”的工作，更多的是一个幕后工作。
...
![chapter43-1](./imgs/chapter43-1.jpg)
...
世纪 90 年代的时候，大家用的硬盘都叫作 **IDE 硬盘**。这个 IDE 不是像 IntelliJ 或者 WebStorm 这样的软件开发集成环境（Integrated Development Environment）的 IDE，而是代表着集成设备电路（Integrated Device Electronics）。也就是说，设备的接口电路直接在设备上，而不在主板上。我们需要通过一个线缆，把集成了接口的设备连接到主板上去。

![chapter43-2](./imgs/chapter43-2.png)
...
![chapter43-3](./imgs/chapter43-3.jpeg)
...
![chapter43-4](./imgs/chapter43-4.jpeg)
...
![chapter43-5](./imgs/chapter43-5.png)
...
<h1 id="chapter44">44 | 理解IO_WAIT：I/O性能到底是怎么回事儿？</h1>
...
![chapter44-1](./imgs/chapter44-1.png)
...
![chapter44-2](./imgs/chapter44-2.png)
...
<h1 id="chapter45">45 | 机械硬盘：Google早期用过的“黑科技”</h1>
...
![chapter45-1](./imgs/chapter45-1.jpg)
...
![chapter45-2](./imgs/chapter45-2.jpg)
...
![chapter45-3](./imgs/chapter45-3.jpg)
...
<h1 id="chapter46">46 | SSD硬盘（上）：如何完成性能优化的KPI？</h1>
...
![chapter46-1](./imgs/chapter46-1.jpeg)
...
![chapter46-2](./imgs/chapter46-2.jpeg)
...
![chapter46-3](./imgs/chapter46-3.jpeg)
...
![chapter46-4](./imgs/chapter46-4.jpeg)
...
![chapter46-5](./imgs/chapter46-5.jpeg)
...
对于 SSD 硬盘来说，数据的**写入**叫作 Program。写入不能像机械硬盘一样，通过覆写（Overwrite）来进行的，而是要先去**擦除**（Erase），然后再写入。
...
![chapter46-6](./imgs/chapter46-6.jpeg)
...
![chapter46-7](./imgs/chapter46-7.jpeg)
...
<h1 id="chapter47">47 | SSD硬盘（下）：如何完成性能优化的KPI？</h1>
...
![chapter47-1](./imgs/chapter47-1.jpeg)
...
![chapter47-2](./imgs/chapter47-2.jpeg)
...
![chapter47-3](./imgs/chapter47-3.jpeg)
...
![chapter47-4](./imgs/chapter47-4.jpeg)
...
<h1 id="chapter48">48 | DMA：为什么Kafka这么快？</h1>
...
![chapter48-1](./imgs/chapter48-1.jpeg)
...
![chapter48-2](./imgs/chapter48-2.jpeg)
...
![chapter48-3](./imgs/chapter48-3.jpg)
...
![chapter48-4](./imgs/chapter48-4.jpg)
...
<h1 id="chapter49">49 | 数据完整性（上）：硬件坏了怎么办？</h1>
...
![chapter49-1](./imgs/chapter49-1.jpeg)
...
![chapter49-2](./imgs/chapter49-2.jpeg)
...
<h1 id="chapter50">50 | 数据完整性（下）：如何还原犯罪现场？</h1>
...
![chapter50-1](./imgs/chapter50-1.jpeg)
...
![chapter50-2](./imgs/chapter50-2.jpeg)
...
![chapter50-3](./imgs/chapter50-3.jpeg)
...
![chapter50-4](./imgs/chapter50-4.jpeg)
...
![chapter50-5](./imgs/chapter50-5.jpeg)
...
![chapter50-6](./imgs/chapter50-6.jpeg)
...
<h1 id="chapter51">51 | 分布式计算：如果所有人的大脑都联网会怎样？</h1>
...
![chapter51-1](./imgs/chapter51-1.png)
...
![chapter51-2](./imgs/chapter51-2.jpeg)
...
![chapter51-3](./imgs/chapter51-3.jpeg)
...
![chapter51-4](./imgs/chapter51-4.jpeg)
...
![chapter51-5](./imgs/chapter51-5.png)
